...
/// marker rather than a name like `env`, which is just as often source
/// (`src/env/`).
fn is_python_env(dir: &Path) -> bool {
...
mod tests {
    ...
    #[test]
    fn env_dirs_are_skipped_only_when_they_are_virtualenvs() {
        let dir = std::env::temp_dir().join(format!("codearch-env-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        for d in ["src/env", "env/lib", "tools/pyenv"] {
            std::fs::create_dir_all(dir.join(d)).unwrap();
        }
        std::fs::write(dir.join("src/env/index.ts"), "export const e = 1;\n").unwrap();
        std::fs::write(dir.join("env/pyvenv.cfg"), "home = /usr\n").unwrap();
        std::fs::write(dir.join("env/lib/site.py"), "x = 1\n").unwrap();
        std::fs::write(dir.join("tools/pyenv/pyvenv.cfg"), "home = /usr\n").unwrap();
        std::fs::write(dir.join("tools/pyenv/mod.py"), "x = 1\n").unwrap();
        let inv = collect(&dir).unwrap();
        let _ = std::fs::remove_dir_all(&dir);
        let rels: Vec<&str> = inv.files.iter().map(|f| f.rel.as_str()).collect();
        assert_eq!(rels, vec!["src/env/index.ts"]);
    }
